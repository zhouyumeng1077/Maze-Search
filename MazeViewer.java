...
//         }
   ...
   */
   private static MazeFrame readMazeFile(String fileName) throws IOException {   
       ...
       Scanner size=new Scanner(first);
       int row=Integer.parseInt(size.next());	        
       int column=Integer.parseInt(size.next());	  
      ...
      MazeCoord startPosition=new MazeCoord(in.nextInt(),in.nextInt());
      MazeCoord endPosition=new MazeCoord(in.nextInt(),in.nextInt());
      in.close();
      return new MazeFrame(maze,startPosition,endPosition);
   }
}